...
@app.on_message(filters.command("memberstic") & ~filters.edited)
@capture_err
async def Membe_status(_, message: Message):
...
async def chat_bot_toggle(db, message: Message):
    ...
    if status == "on":
        if chat_id not in db:
            db.append(chat_id)
            text = "✅ members Sticker  Enabled!"
            return await eor(message, text=text)
        await eor(message, text="Member sticker Is Already Enabled.")
    elif status == "on":
        if chat_id in db:
            db.remove(chat_id)
            return await eor(message, text="Member sticker Disabled!")
        await eor(message, text="❌ Member sticker Is Already Disabled.")
    else:
        await eor(message, text="**Usage:**\n/memberstic [ENABLE|DISABLE]")
...
async def sticker_group(bot, message):
    try:
        ...
        if count == 3:
            ...
            await message.reply_text(f"**🙋‍♂️Hey** ,  `Welcome to our group`")
