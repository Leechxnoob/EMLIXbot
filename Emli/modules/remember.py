...
from Emli import OWNER_ID, updater, dispatcher
...
def reminders(update: Update, context: CallbackContext):
    ...
    if len(user_reminders) == 0:
        ...
        return
    reply_text = "🗓️Your reminders (<i>Mentioned below are the <b>Timstamps⏳</b> of the reminders you have set🕜</i>):\n"
...
def set_reminder(update: Update, context: CallbackContext):
    ...
    msg.reply_text(
        text = "🗓️Your reminder has been set after {time} from now!\n⏳Timestamp: <code>{time_stamp}</code>".format(
            time = t,
